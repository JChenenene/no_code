            ...
            >
              ...
              <div class="workflow-step-body">
                ...
                <div class="workflow-step-desc">
                  {{ item.outputSummary || item.inputSummary || item.stage }}
                </div>
              </div>
            </div>
          </div>
...
const generationMode = ref<'standard' | 'workflow_v2'>('standard')
const generationModeOptions = [
  { label: '普通生成', value: 'standard' },
  { label: 'V2 工作流', value: 'workflow_v2' },
]
...
const fetchAppInfo = async () => {
  ...
  try {
    ...
    if (res.data.code === 0 && res.data.data) {
      ...
      await restoreLatestSucceededWorkflowPreview()
...
const restoreLatestWorkflowRun = async () => {
  if (!appId.value || !isOwner.value) return
  try {
    const res = await getLatestWorkflowRunDetail({ appId: appId.value as unknown as number })
    ...
    workflowStatus.value = run.status === 'succeeded'
        ? 'succeeded'
        : run.status === 'failed'
            ? 'failed'
    ...
    const restoredSteps = (detail?.steps || []).map((step) => ({
      ...
      outputSummary: step.errorMessage || step.outputSummary,
    }))
    workflowTimeline.value = restoredSteps.length > 0
        ? restoredSteps
        : [
            {
              agentName: 'WorkflowV2',
              stage: 'RESTORED',
              status: run.status.toUpperCase(),
              outputSummary: run.errorMessage || run.prompt || '已恢复最近一次工作流记录',
            },
          ]
  } catch (error) {
...
const restoreLatestSucceededWorkflowPreview = async () => {
  if (!appId.value || !isOwner.value) return
  try {
    const res = await getLatestSucceededWorkflowRun({ appId: appId.value as unknown as number })
    ...
    console.warn('恢复 V2 最近成功预览失败:', error)
  }
}
...
const generateCode = async (userMessage: string, aiMessageIndex: number) => {
  if (generationMode.value === 'workflow_v2') {
    await generateCodeWithWorkflowV2(userMessage, aiMessageIndex)
    return
  }
...
const parseWorkflowEvent = (rawData: string) => {
  if (!rawData) return null
  try {
    return JSON.parse(rawData)
  } catch (error) {
    console.error('解析 V2 工作流事件失败:', error, rawData)
    return null
  }
}

const buildWorkflowCompletedMessage = (data: any) => {
  const finalStatus = data?.finalStatus === 'SUCCESS' ? '成功' : '未通过'
  const verificationSummary = data?.verificationSummary ? `\n\n验证: ${data.verificationSummary}` : ''
  const fixSummary = data?.fixSummary ? `\n\n修复: ${data.fixSummary}` : ''
  const issues = Array.isArray(data?.verificationIssues) && data.verificationIssues.length > 0
...
const updatePreview = () => {
  if (appId.value) {
    if (latestWorkflowPreviewUrl.value) {
      previewUrl.value = latestWorkflowPreviewUrl.value
      previewReady.value = true
      return
    }
...
const buildWorkflowPreviewUrl = (runId: number | string) => {
  const codeGenType = appInfo.value?.codeGenType || CodeGenTypeEnum.HTML
  const baseURL = request.defaults.baseURL || API_BASE_URL
  const normalizedBaseURL = baseURL.endsWith('/') ? baseURL.slice(0, -1) : baseURL
  const basePreviewUrl = `${normalizedBaseURL}/static/${appId.value}/${runId}/${codeGenType}/`
  if (codeGenType === CodeGenTypeEnum.VUE_PROJECT) {
    return `${basePreviewUrl}dist/index.html`
  }
...
.ai-message .message-content :deep(.markdown-content > :first-child) {
  margin-top: 0;
}

.ai-message .message-content :deep(.markdown-content > :last-child) {
  margin-bottom: 0;
}
