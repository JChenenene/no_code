...
import { getAppVoById, getLatestSucceededWorkflowRun, updateApp, updateAppByAdmin } from '@/api/appController'
...
const openPreview = async () => {
  if (appInfo.value?.codeGenType && appInfo.value?.id) {
    try {
      const res = await getLatestSucceededWorkflowRun({ appId: appInfo.value.id })
      const run = res.data.data
      if (run?.previewUrl) {
        window.open(normalizePreviewUrl(run.previewUrl), '_blank')
        return
      }
    } catch (error) {
      console.warn('获取 V2 预览地址失败，回退到旧预览地址：', error)
    }
