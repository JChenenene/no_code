...
@Slf4j
public class CodeAuthorAgent {

    private static final String AGENT_NAME = "CodeAuthorAgent";
    ...
    public Map<String, Object> execute(MessagesState<String> state) {
        ...
        sessionState.setFixPlanArtifact(null);

        CodeGenTypeEnum codeGenTypeEnum = resolveCodeGenType(sessionState.getTaskSpec());
        String authorPrompt = workflowV2PromptComposer.composeAuthorPrompt(sessionState);
        ...
        try {
            boolean buildAfterGenerate = codeGenTypeEnum != CodeGenTypeEnum.VUE_PROJECT;
            ...
            if (!generatedArtifactSupport.directoryExists(generatedCodeDir)
                ...
                artifactBuilder.summary("代码生成结束，但未检测到有效代码产物")
                        .errorMessage("未找到有效的生成代码目录或关键文件");
            } else {
                List<String> keyFiles = generatedArtifactSupport.listKeyFiles(generatedCodeDir, 12);
                artifactBuilder.keyFiles(keyFiles)
                        .summary("代码生成完成，产物目录: %s".formatted(generatedCodeDir));
            }
        } catch (Exception e) {
            log.error("requestId={}, agent={}, 代码生成失败: {}",
                    sessionState.getRequestId(), AGENT_NAME, e.getMessage(), e);
            artifactBuilder.summary("代码生成失败")
        ...
        sessionState.finishAgentExecution(
                ...
                StrUtil.blankToDefault(codeArtifact.getSummary(), "代码生成完成"),
                "unavailable"
        );
