package com.lingchuang.ai.utils;
...
public class WebScreenshotUtils {
    ...
    private static void compressImage(String originImagePath, String compressedImagePath) {
        ...
        try {
            File originImageFile = FileUtil.file(originImagePath);
            File compressedImageFile = FileUtil.file(compressedImagePath);
            BufferedImage sourceImage = ImageIO.read(originImageFile);
            if (sourceImage == null) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "读取原始截图失败");
            }
            ...
            if (!writers.hasNext()) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "未找到 JPG 图片编码器");
            }
            ImageWriter writer = writers.next();
            try (ImageOutputStream imageOutputStream = ImageIO.createImageOutputStream(compressedImageFile)) {
            ...
            if (ImageIO.read(compressedImageFile) == null) {
                throw new BusinessException(ErrorCode.SYSTEM_ERROR, "压缩后的截图无法解码");
            }
        } catch (Exception e) {
