...
public class AiServiceTokenStream implements TokenStream {
    ...
    public void start() {
        ...
        ChatExecutor.StreamingToSynchronousBuilder chatExecutorBuilder = ChatExecutor.builder(context.streamingChatModel)
                .errorHandler(errorHandler);
        chatExecutorBuilder.chatRequest(chatRequest);
        chatExecutorBuilder.invocationContext(invocationContext);
        if (context.eventListenerRegistrar != null) {
            chatExecutorBuilder.eventListenerRegistrar(context.eventListenerRegistrar);
        }
        ChatExecutor chatExecutor = chatExecutorBuilder.build();
